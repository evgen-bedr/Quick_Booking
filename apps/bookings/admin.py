...
class BookingAdmin(admin.ModelAdmin):
    list_display = (
        'booking_link', 'user_link', 'rental_link', 'start_date', 'end_date', 'price', 'status', 'created_at', 'updated_at'
    )
    ...
    def has_change_permission(self, request, obj=None):
        # Ограничить изменение бронирований только администратором
        if not request.user.is_superuser:
            return False
        return super().has_change_permission(request, obj)

    def has_delete_permission(self, request, obj=None):
        # Ограничить удаление бронирований только администратором
        if not request.user.is_superuser:
            return False
